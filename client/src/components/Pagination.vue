<template>
  <nav aria-label="Page navigation example page_top">
    <ul class="pagination justify-content-center">
      <li class="page-item" v-if="current_page > 1">
        <button class="page-link" @click="change_page(current_page - 1)">Previous</button>
      </li>
      <li v-for="page in pages" class="page-item" :class="{ active: current_page == page }">
        <button class="page-link" @click="change_page(page)">{{ page }}</button>
      </li>
      <li class="page-item" v-if="current_page < total_page">
        <button class="page-link" @click="change_page(current_page + 1)">Next</button>
      </li>
    </ul>
  </nav>
</template>
