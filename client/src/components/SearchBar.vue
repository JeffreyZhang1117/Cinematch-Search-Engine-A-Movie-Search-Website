...
export default {
  ...
  data: function () {
    return {
      ...
      searchText: '',
      searchFilter: 'Title',
      filter_list: ['Title', 'Genre', 'Celebrity', 'Plot'],
      myData: [],
      tt: '',
      now: -1,
      listgroup: null,
  ...
  methods: {
    ...
    async search() {
      const searchURL = '/home/search?' + this.searchFilter + '=' + this.searchText
      ...
      if (result == false) {
        ...
        if (this.searchFilter == 'All') {
          ...
          let { sort: sortBy } = this.$route.query
          sortBy = sortBy || this.sort[0]
          this.$router.push({
            path: '/searchAll',
            query: { [this.searchFilter]: this.searchText, sort: sortBy },
          })
        } else {
