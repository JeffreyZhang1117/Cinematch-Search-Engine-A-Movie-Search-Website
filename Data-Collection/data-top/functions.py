...
def get_tconst(url):
    ...
    html_text = scrape_html(url)
    info = re.findall('> <img alt="(.*?)"\nclass="(.*?)"\nloadlate="(.*?)"\ndata-tconst="(.*?)"\nheight="(.*?)"\nsrc="(.*?)"\nwidth="(.*?)" />', html_text)
    for a,b,c,d,e,f,g in info:
        page_tconst_list.extend([d])
...
if __name__ == '__main__':
    # pop_tconst = get_all_pop_tconst
    # print(pop_tconst)
    url = 'https://www.imdb.com/search/title/?title_type=feature&year=2010-01-01,2022-11-31&start=9950&ref_=adv_prv'
    html_text = scrape_html(url)
    print(html_text)
    # a = get_tconst(url)
    # b = get_info1(html_text)
    # print(b[0][3])
    # print(len(b))
    # c = []
    ...
    # print(html_text)
    # page_info50_list = get_info50(url)
    # print(page_info50_list)
